...
/// ```
///
/// Specifying a cryptographic provider introduces a `Result` that must be unwrapped,
/// because the config builder checks for consistency of the provider. For instance, it's an error to
/// have a `CryptoProvider` that has no cipher suites.  For pre-made providers like the built-in ones,
/// this is not an issue and you can `.unwrap()` this error.  However, if you construct or customize
/// your own `CryptoProvider`, you should handle this error.
///
